...
- 首次接入默认先收齐：
  ...
  - `JUMPSERVER_USERNAME`
  - `JUMPSERVER_PASSWORD`
  - JumpServer 部署服务器的资产名或 IP，对应 `JMS_EXEC_ASSET_NAME`
...
- 常见落盘项：
  ...
  - 命令巡检目标：`JMS_EXEC_ASSET_NAME`、`JMS_EXEC_ACCOUNT_NAME`
  - 旧版报告节点：`JMS_SYSTEM_TARGETS`
  - 数据库覆盖：`DB_*` 或 `JMS_DB_*`
  - 首次确认 `JMS_EXEC_ASSET_NAME` 和 `JMS_EXEC_ACCOUNT_NAME` 后，也可以同步写一份 `JMS_SYSTEM_TARGETS`
