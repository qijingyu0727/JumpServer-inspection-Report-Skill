...
- 依赖安装：
  ...
  - 安装 skill 后建议先执行 `python3 -m pip install -r requirements.txt`
  - 命令巡检首次仍需执行 `python3 -m playwright install chromium`
...
- 未配置 `JMS_SYSTEM_TARGETS` 时，会优先复用 `JMS_EXEC_ASSET_NAME / JMS_EXEC_ACCOUNT_NAME`
...
- `report ... html` 默认输出旧版巡检报告字段级对齐 HTML
- `report ... html --style modern` 才会走新版控制台样式
