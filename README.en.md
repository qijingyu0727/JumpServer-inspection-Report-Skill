...
HTML reports now default to `legacy`, which uses the official `jms-inspect-go` engine over remote SSH. `modern` provides a lighter control-room brief in the same JumpServer-themed visual system.
...
- `bootstrap` creates `runtime/profiles/prod.env`, stores Chromium under `runtime/.playwright-browsers`, and prepares `runtime/bin/jms_inspect`
- `exec` now prefers an already installed system Chrome/Chromium and only downloads Playwright Chromium when no local browser is available
...
- The installer now prefers more direct Chromium hosts such as `https://storage.googleapis.com/chrome-for-testing-public`; set `PLAYWRIGHT_CHROMIUM_DOWNLOAD_HOST` to pin a custom source
- Legacy full reports upload the official binary to the JumpServer host over SSH; a sidecar `_official_bundle/` directory is saved next to the final HTML with the retrieved HTML/JSON/Excel artifacts

## Features

- Formal inspection reports in `html` and `markdown`
- Full `legacy` data path now runs the official `jms-inspect-go` binary on the JumpServer host for system, database, Redis, and service inspection
...
- `JMS_OFFICIAL_SSH_USERNAME/JMS_OFFICIAL_SSH_PASSWORD` are the SSH credentials used by the official legacy engine
- The default report style is `legacy`, which means the full inspection edition
- `JMS_LEGACY_PROVIDER=official` means `legacy` HTML is generated by the official remote inspection engine
...
`self-test` now checks JumpServer API, command execution readiness, and the official legacy binary, SSH reachability, remote `config.txt`, plus official `--check-only`.

If the tool asks for org, host, official SSH, or database config, write it back into the profile:
...
- `legacy` defaults to `JMS_LEGACY_PROVIDER=official`, which runs the official inspection on the JumpServer host over SSH and retrieves HTML/JSON/Excel artifacts
- If needed, you can explicitly fall back to the older Python data path with `JMS_LEGACY_PROVIDER=python`
...
- Official remote execution depends on `paramiko`, and the bundled binary asset lives at `assets/bin/linux_amd64/jms_inspect`
- Chromium for `exec` prefers a locally installed Chrome/Chromium first, and otherwise retries across multiple download hosts
