...
description: 作为 JumpServer 巡检工具箱处理“巡检报告/日报/月报”“legacy 旧版巡检报告”“异常登录与 Top 10 分析”“单机负载与谁在使用”“按 Word/PDF 模板补全巡检报告”“飞书载荷与定时巡检”等请求。首次默认要求用户先提供 JUMPSERVER_URL、JUMPSERVER_USERNAME、JUMPSERVER_PASSWORD、JumpServer 部署服务器的资产名或 IP，以及连接该服务器的账号名；确认后写回 env/profile 复用。
---
...
- 首次接入默认先收齐这 5 项：
  ...
  - `JUMPSERVER_USERNAME`
  - `JUMPSERVER_PASSWORD`
  - `JMS_EXEC_ASSET_NAME`：JumpServer 部署服务器的资产名或 IP
...
- 理想执行链路固定为一条：
  ...
  - 拿到这 5 项后立即写回 env/profile
  - 再连接该服务器执行命令、读取 `/opt/jumpserver/config/config.txt` 并补齐数据库查询
- 其他缺少的运行信息必须追问，不做隐式猜测。问到后立即写回 env/profile，供后续问答直接复用：
  - `JUMPSERVER_ORG` / `JMS_DEFAULT_ORG_NAME`
  - `JMS_EXEC_ASSET_NAME` / `JMS_EXEC_ACCOUNT_NAME`
  - `JMS_SYSTEM_TARGETS`
  - `DB_*` / `JMS_DB_*`
...
- 正式报告仍优先走 `html`，且默认按 `legacy` 输出完整数据面；只有用户明确要求新版视觉时才切 `modern`。分析类请求默认直接返回结构化内容，不强制落文件。
- 用户要求正式巡检报告、旧版巡检报告、带系统信息和数据库统计的完整报告时，默认走 `report ... html`（即 `legacy`）：
  - 系统信息从 JumpServer 节点本机采集
  ...
  - 用户明确要求新版控制台视觉时，再追加 `--style modern`
...
- 能力缺失时优先尝试自动安装：
  - skill 安装阶段应先安装 `requirements.txt` 中声明的 `PyMySQL[rsa]`、`cryptography`、`playwright`、`python-docx`、`pypdf`
